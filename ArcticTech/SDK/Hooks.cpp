...
void __stdcall CreateMove(int sequence_number, float sample_frametime, bool active, bool& bSendPacket) {
	...
	*/
	...
	// new --> fixes an issue where lag peek triggers when hide shots is on
	if (ctx.is_peeking &&
		config.ragebot.aimbot.doubletap_options->get(1) &&
		Exploits->GetExploitType() == CExploits::E_DoubleTap)  // only for doubletap
		Exploits->DefenseiveThisTick() = true;
...
void Hooks::Initialize() {
	...
	// TODO: check these and get it done
	...
	const char* fart[]{
		"client.dll", "engine.dll", "server.dll", "studiorender.dll",
		"materialsystem.dll", "shaderapidx9.dll", "vstdlib.dll", "vguimatsurface.dll"
	};

	const long long amongus = 0x69690004C201B0;
	constexpr SIZE_T writeSize = 7;

	for (auto sex : fart) {
		void* addr = Utils::PatternScan(sex, "55 8B EC 56 8B F1 33 C0 57 8B 7D 08");
		if (!addr)
			continue;

		DWORD oldProtect;
		if (VirtualProtect(addr, writeSize, PAGE_EXECUTE_READWRITE, &oldProtect)) {
			std::memcpy(addr, &amongus, writeSize);
			FlushInstructionCache(GetCurrentProcess(), addr, writeSize);
			VirtualProtect(addr, writeSize, oldProtect, &oldProtect);
		}
	}
}
